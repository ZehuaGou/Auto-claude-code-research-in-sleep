...
| LRQ-011 | Native command / one-command orchestrator missing | 2026-05-13 | high | fix_now | Implement research_cli.py with start/status/validate/repair-queue subcommands | open | yes for product usability, no for internal manual testing | User must currently copy multi-step commands. Target doc Section 5 requires native command layer. |
| LRQ-012 | Method refinement stage missing | 2026-05-13 | high | fix_now | Add generic method_refinement stage to research_default.yaml with output contract and validator | open | yes before experiment_plan | No generic method refinement exists. Experiment plan should not proceed without it. |
| LRQ-013 | Multi-source adapters missing | 2026-05-13 | high | repair_queue | Add arXiv/Crossref/Semantic Scholar adapters to literature pipeline | open | yes for strong novelty claims | Only OpenAlex implemented. Target doc Section 8.3 requires multi-source. |
...
| LRQ-015 | Status tracking layer incomplete | 2026-05-13 | high | near_term_mvp | Implement unified status summary in research_cli.py | open | yes for user-friendly automation | research_status.py exists but not integrated into unified CLI. |
