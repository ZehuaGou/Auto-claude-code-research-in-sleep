...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via Codex MCP for plan review, figure review, writing review, and improvement loop. All reviewer/auditor calls must follow the global trusted role execution protocol (`shared-references/trusted-role-execution.md`). The reviewer/auditor role must be executed via `tools/trusted_role_runner.py`, not by the external agent directly.
