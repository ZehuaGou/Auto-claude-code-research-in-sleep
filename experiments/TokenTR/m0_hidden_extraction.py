...
def normalize_ragtruth_row(row, tok, max_tokens=2048):
    """
    Normalize a RAGTruth_Xtended row for TokenTR.
    Does NOT generate token_labels here — that happens in build_ragtruth_labels()
    inside the extraction loop where the ACTUAL comp_len (post-truncation) is known.
    This preserves ground_truth.annotation for use by build_ragtruth_labels().

    Returns (normalized_row, error_msg) where normalized_row includes:
    - sample_id
    - prompt
    - response
    - ground_truth (preserved for label generation in extraction loop)
    """
    try:
        ...
        # Verify annotation field exists and is list-like
        char_spans = gt["annotation"]
        if not isinstance(char_spans, list):
            return None, f"sample {sample_id}: ground_truth.annotation is not a list"

        return {
            "sample_id": sample_id,
            "prompt": prompt,
            "response": response,
            "ground_truth": gt,  # PRESERVED — used by build_ragtruth_labels() in extraction
        }, None
    except Exception as e:
        return None, str(e)


def build_ragtruth_labels(row, tok, actual_response_len):
    """
    Build token labels from a RAGTruth row, returning a list of length actual_response_len.
    This is called in the extraction loop with the ACTUAL response token count after
    combined-sequence truncation, so labels are always aligned to hidden states.

    CRITICAL: must tokenize the response with the SAME truncation limit used in extraction.
    If prompt fills most of max_tokens, response gets truncated in combined_ids.
    We re-tokenize with max_length=actual_response_len to match what the model actually saw.
    """
    response = row["response"]
    gt = row.get("ground_truth", {})
    char_spans = gt.get("annotation", [])

    # Tokenize response with EXACTLY actual_response_len tokens to match extraction
    enc = tok(response, add_special_tokens=False, return_offsets_mapping=True,
               truncation=True, max_length=actual_response_len)
    offsets = enc["offset_mapping"]  # [(start, end), ...] per token

    # Build labels of EXACTLY actual_response_len tokens
    token_labels = [0] * actual_response_len
    for start_char, end_char in char_spans:
        for t in range(min(actual_response_len, len(offsets))):
            tok_start, tok_end = offsets[t]
...
def validate_dataset_schema(rows, label_mode, dataset_path, dataset_format=None):
    """
    ...
    # RAGTruth_Xtended format builds labels inline in the extraction loop (build_ragtruth_labels)
    # so token_labels field is not required on the normalized row
    is_ragtruth_xtended = (dataset_format == "ragtruth_xtended")

    if label_mode in FORMAL_LABEL_MODES:
        if label_mode == "token" and not has_token_labels and not is_ragtruth_xtended:
            return False, f"--label_mode token but no 'token_labels' field found", {}
        if label_mode == "token" and is_ragtruth_xtended:
            pass  # labels built inline in extraction loop
...
def main():
    ...
    parser.add_argument("--model_name_or_path", type=str, default=None,
                        help="Local path or HuggingFace model ID. "
                             "Overrides --model_size. "
                             "Example: external_data/models/Qwen2.5-0.5B-Instruct")
    ...
    MODEL = args.model_name_or_path or (MODEL_NAME_7B if args.model_size == "7b" else MODEL_NAME)
    ...
    if dataset_format == "ragtruth_xtended" or (dataset_format is None and detect_ragtruth_xtended(rows[0])):
        ...
        print(f"  ground_truth.annotation preserved: {all('ground_truth' in r and 'annotation' in r.get('ground_truth', {}) for r in rows)}", flush=True)
        print(f"  (token_labels built inline in extraction loop for ragtruth_xtended)", flush=True)
    ...
    # ── Schema validation ──────────────────────────────────────────────────────
    is_valid, err_msg, metadata = validate_dataset_schema(rows, args.label_mode, args.dataset_path, dataset_format)
    if not is_valid:
    ...
    for i, row in enumerate(rows):
        ...
        try:
            ...
            if args.label_mode in FORMAL_LABEL_MODES:
                if dataset_format == "ragtruth_xtended" and args.label_mode == "token":
                    # Build labels inline with ACTUAL comp_len (post-truncation)
                    labels = build_ragtruth_labels(row, tok, comp_len)
                else:
